...
def recuperar_dados_arqtxt(tab_funcionario):
    tab_funcionario.clear()
    nome_arq = input("Digite o nome do arquivo texto (com extensão): ")
    with open(nome_arq,"r",encoding="windows-1252") as arq:
        lista = arq.readlines()
    for dados in lista:
        lista_func = dados.split("*")
        funcionario = {'Codigo':int(lista_func[0]),'Nome':lista_func[1],'Idade':int(lista_func[2]),'Salario':float(lista_func[3])}
